...
import { readQueue, decide } from "../queue.mjs";
...
async function bench(prefix, count) {
  ...
  for (let i = 1; i <= count; i += 1) {
    const id = `q-20260801-0900-${i}`;
    ids.push(id);
    lines.push(`- ${id} | chat | personal | p${i}@example.com | Note ${i}`);
  }
...
test("paging is exact at and around the page boundary", async () => {
  ...
  for (const n of [120, 121, 239, 240, 241]) {
    ...
    const { offered, pages, counted } = await walk(paths);
    const repeated = [...offered.values()].filter((c) => c > 1).length;
    const expectedPages = Math.ceil(n / 120);
    if (counted !== n || offered.size !== counted || repeated || pages.length !== expectedPages) {
      wrong.push(`${n}: counted ${counted}, reached ${offered.size}, ${repeated} repeated, ${pages.length} pages`);
    }
  }
...
test("the load control promises only the next bounded response", async () => {
  const { fetchImpl } = station(300, 120);
  const view = await loadQueueView(ORDERLY, { fetchImpl });
  await view.checkQueue();
  assert.equal(view.stack.querySelector(".queue-more").querySelector("BUTTON").textContent, "Load 120 more");
});
