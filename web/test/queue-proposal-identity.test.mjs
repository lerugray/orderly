...
import { readQueue, observeProposal, observeDraft, decide } from "../queue.mjs";
...
test("durable completion identity keeps effect-significant chat variants separate", async () => {
  const { pendingPath, statePath } = await bench("orderly-identity-completed-variant-");
  await writeFile(pendingPath, "");
  const first = await observeProposal({
    statePath, desk: "coordinator", card: { ...chatCard("-"), location: "Room A" },
  });
  const second = await observeProposal({
    statePath, desk: "coordinator", card: { ...chatCard("-"), location: "Room B" },
  });
  await decide({
    pendingPath, statePath, id: first, decision: "approve",
    execute: async () => ({ eventId: "made-1", action: "create", account: "personal" }),
  });
  const queue = await readQueue({ pendingPath, statePath });
  assert.deepEqual(queue.pending.map((item) => item.id), [second],
    "completing Room A must not suppress the otherwise-identical Room B effect");
});
