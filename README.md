...
- Supports named persistent agents on an `/agents` page: create one with a name, a description, and a handle, and it gets its own standing thread and memory on the desk. Creation is credential-free by construction — a name or description that reads like a path, a URL, an environment assignment, or a credential is refused, not stored — and every change that matters to trust shows a plain confirmation of exactly what it grants before you confirm it.
- Lets a seat run on a model you host yourself — anything speaking the OpenAI-compatible chat API, on this machine, your LAN, or your tailnet — probed end to end before it's offered, and honest about what it's trusted with: full chat and research, a single tool call only where a probe has actually watched that engine make one, nothing further pretended.

Further specifications in [`docs/`](docs/) set what's next: per-agent sandboxes, delegation to the mail and research agents, group agents, and attaching a named agent to its own Telegram bot; the bounded-execution harness the chained-task model tier needs; and, banked as direction rather than a spec yet, per-agent connectors to outside services.
